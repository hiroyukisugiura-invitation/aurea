...
app.post("/chat", async (req, res) => {
  ...
  const reportsBlock = GPT_COMPAT_MODE ? "" : buildReportsBlock(map);
    ...
    const visionSystem = userParts.some(p => p && p.type === "input_image")
      ? [
          "Vision analysis (GPT-like, helpful-first):",
          "",
          "Hard rules:",
          "- Do NOT identify the person or guess their name/identity.",
          "- Do NOT claim sensitive attributes (race, religion, etc.).",
          "- Do NOT invent objects/text that are not visible.",
          "",
          "Important:",
          "- Never refuse the whole request just because a person is in the image.",
          "- If the user asks for identification: say you cannot identify, then continue with a helpful description.",
          "- You MAY describe appearance, expression, pose, composition, lighting, clothing (non-identifying), and overall mood.",
          "",
          "Output (keep natural, not a rigid report):",
          "- Start with a short, human-sounding description (2–5 lines).",
          "- Then 3–8 bullets of visible facts (only what is visible).",
          "- If useful, add 2–5 suggestions or observations grounded in the image.",
...
const gptCompatVisionHelpful = [
  ...
  "- Do NOT compress into short or minimal answers.",
  "- Write as if explaining the image to another human in conversation.",
  "",
